...
## 🛠️ 21-Sector Architecture Overview
...
| **13. Bus Latency & ASPM** | PCIe & USB 3.2 | PCIe ASPM set to `performance` on AC; USB autosuspend disabled on AC; Intel UHD iGPU unlocked to 1250 MHz boost clock. |
...
| **17. Gaming & Throughput** | Mesa & Network Stack | GameMode integration; socket buffer expansion (`rmem_max = 16777216`, `wmem_max = 16777216`); TCP BBR maximum throughput for low ping gaming and streaming. |
| **18. Driver Resilience** | Linux Kernel 7.2 | Kernel boot parameters embedded via grubby: `split_lock_mitigate=0 nowatchdog transparent_hugepage=madvise loglevel=3`; early initramfs force driver inclusions (`intel_lpss_pci pinctrl_tigerlake i2c_designware_platform i2c_hid_acpi`). |
| **19. Webcam Optimization** | Chicony FHD WebCam (`04f2:b650`) | Power line anti-flicker frequency locked to 50 Hz matching regional AC electricity; `uvcvideo` driver options `nodrop=1 quirks=128` (prevents frame drops during lighting changes & fixes bandwidth calculation); WirePlumber camera priority rule set; user added to `video,render` groups; USB autosuspend shielded. |
| **20. OS Integrity & Audio Latency** | System & Audio Bus | System package database and systemd unit health audited; Realtek ALC269VC audio bus power-gating latency zeroed (`options snd_hda_intel power_save=0 power_save_controller=N`) eliminating DAC clicks, pops, and sleep delays on stream start. |
| **21. Hardware Mitigations** | Raptor Lake-U Platform | System-wide VA-API hardware video acceleration policy deployed (`LIBVA_DRIVER_NAME=iHD`, `VDPAU_DRIVER=va_gl`) with Firefox system policies (`media.ffmpeg.vaapi.enabled=true`) stopping 100% CPU spikes during 1080p/4K YouTube playback; pure ZRAM sleep protection via masking broken S4 hibernation targets (`hibernate.target`, `hybrid-sleep.target`). |
